...
float PID1outArray[200];
float PID2outArray[2000];
float PID3outArray[200];
...
__interrupt void cla1_task2_isr(void)
{
    ...
    if(b<200){
        PID2outArray[b]=temp2; //PID2_Out;
        b++;
        }

    EALLOW;
    // Comp2Regs.RAMPMAXREF_SHDW=  65535;  //PID2_Out;
    EDIS;
}
...
__interrupt void cla1_task3_isr(void)
{
    ...
    if(c<200){
          PID3outArray[c]=temp3;
          c++;
          }

   EALLOW;
   // Comp3Regs.RAMPMAXREF_SHDW=  65535;  //PID3_Out;
   EDIS;
}
